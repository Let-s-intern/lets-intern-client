...
import MenuContentsDropdown from '../my-challenge/mission/ui/MenuContentsDropdown';
import ParsedCommentBox from '../my-challenge/ui/ParsedCommentBox';
interface Props {
        ...
        {isFeedbackConfirmed && (
          <div className="mt-4">
            ...
            <div className="mt-2">
              <ParsedCommentBox
                className="rounded-md bg-[#F2F2F2] px-8 py-6 text-sm"
                comment={`(미션 피드백 페이지로 이동)[/old/challenge/${applicationId}/${programId}/missions/${missionDetail.id}/feedback]`}
                openInNewTab={false}
              />
            </div>
          </div>
