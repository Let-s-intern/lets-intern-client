...
const renderAccordion = (
  props: Partial<Parameters<typeof PriceDetailAccordion>[0]> = {},
) =>
  render(
...
describe('PriceDetailAccordion', () => {
  it('접힌 상태로 시작한다', () => {
    ...
    expect(screen.queryByText('스탠다드 플랜 금액')).not.toBeInTheDocument();
  });
  ...
  it('펼치면 판매가 차액 행과 안내 세 줄을 보여주고 다시 접힌다', async () => {
    ...
    expect(
      screen.getByText('스탠다드 플랜 금액').nextSibling,
  ...
  it('현재 플랜 금액이 0원이어도 상위 플랜 금액 - 현재 플랜 금액 = 추가 결제 금액으로 보인다', async () => {
    renderAccordion({
      currentSalePrice: 0,
      targetPlanType: 'PREMIUM',
      targetSalePrice: 2000,
    ...
    ).toHaveTextContent('2,000원');
    expect(
      screen.getByText('현재 베이직 플랜 금액').nextSibling,
    ).toHaveTextContent('- 0원');
    expect(screen.getByText('추가 결제 금액').nextSibling).toHaveTextContent(
      '= 2,000원',
    );
  });
