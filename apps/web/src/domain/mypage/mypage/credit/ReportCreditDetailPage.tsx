...
import {
  ...
  reportPaymentDetailQueryOptions,
} from '@/api/report';
...
const ReportCreditDetailContent = ({
  applicationId,
  paymentId,
}: {
  ...
  const { data: reportPaymentDetail } = useSuspenseQuery(
    reportPaymentDetailQueryOptions(Number(applicationId)),
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex w-full flex-col items-start justify-center gap-y-6">
        ...
        {!userData ? (
          userDataIsLoading ? (
            <p className="text-neutral-0">참여자 정보를 불러오는 중입니다.</p>
          ) : userDataIsError ? (
            <p className="text-neutral-0">
        ...
        <div className="flex w-full flex-col items-start justify-start">
          <div className="border-neutral-0 flex w-full flex-col items-center justify-start gap-y-2 border-y-[1.5px] px-3 py-5">
            <div className="text-neutral-0 flex w-full items-center justify-start gap-3 font-bold">
              ...
              <div className="flex grow items-center justify-end">
                {reportPaymentDetail.tossInfo &&
                typeof reportPaymentDetail.tossInfo.totalAmount === 'number' &&
                typeof reportPaymentDetail.tossInfo.balanceAmount === 'number'
                  ? (isCanceled
          ...
          <div className="flex w-full flex-col p-3">
            ...
            {reportPaymentDetail.tossInfo &&
              ...
              reportPaymentDetail.tossInfo.totalAmount -
                reportPaymentDetail.tossInfo.balanceAmount !==
                (reportPaymentDetail.reportPaymentInfo.reportRefundPrice ?? 0) +
                  (reportPaymentDetail.reportPaymentInfo.feedbackRefundPrice ??
                    0) && (
                <>
                  <ReportCreditRow
                    title={`관리자 환불금액`}
                  ...
                  <div className="flex w-full flex-col">
                    {reportPaymentDetail.tossInfo.cancels &&
                      reportPaymentDetail.tossInfo.cancels
                        .filter((cancel) => cancel.cancelAmount !== totalRefund)
                        .map((cancel, index) => (
                          <ReportCreditSubRow
                            key={index}
