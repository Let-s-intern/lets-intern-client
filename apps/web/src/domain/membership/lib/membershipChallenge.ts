...
const MEMBERSHIP_GUIDE_URL_BY_CHALLENGE_ID: Record<number, string> = {
  ...
  // [LC-3294] 405 — 마케팅 취준 올인원 패스. 하반기 멤버십과 다른 상품이라 가이드도 별개다.
  //
  // 이 키는 운영 `NEXT_PUBLIC_MEMBERSHIP_CHALLENGE_ID` 와 같은 값이어야 한다. 둘 중 하나만
  // 바꾸면 조용히 어긋난다 — env 만 바꾸면 신규 구매자의 마이페이지 버튼이 '가이드 확인'
  // 대신 '대시보드 입장' 이 되어 노션에 닿을 길이 없어지고, 키만 바꾸면 랜딩이 계속 옛
  // 챌린지를 판다. 어느 쪽도 에러가 나지 않는다.
  //
  // 로컬·QA env 는 다른 챌린지(3)를 가리키므로 이 짝맞춤을 단위 테스트로 잡을 수 없고,
  // 로컬에서 마이페이지 가이드 버튼을 확인하려면 여기에 그 ID 를 잠시 더해야 한다.
  405: 'https://letsintern.notion.site/3d75e77cbee18036810be9eb23a47671',
};
