...
const items: ComparePriceItem[] = [
  ...
  { challengeType: 'ETC', name: '인적성 챌린지', price: 79000 },
];

describe('IndividualPurchaseCard', () => {
  it('항목 이름과 현재 할인가, 합계를 그린다', () => {
    render(
      <IndividualPurchaseCard items={items} total={207500} isLoading={false} />,
    );
    ...
    expect(screen.getByText('207,500원')).toBeInTheDocument();
  ...
  it('로딩 중에는 금액 대신 스켈레톤을 그린다 (폴백 숫자를 내지 않는다)', () => {
    ...
    expect(screen.queryByText('128,500')).not.toBeInTheDocument();
  ...
  it('가격이 null 인 항목은 안내 문구를 내고 합계 개수에서 빠진다', () => {
    ...
    expect(screen.getByText('128,500원')).toBeInTheDocument();
  });
});
