...
describe('toNotionEmbedUrl', () => {
  ...
  test('쿼리스트링 포함 publish URL → /ebd/<id> 로 정규화(쿼리 제거)', () => {
    expect(
      toNotionEmbedUrl(
  ...
  test('페이지 이름이 prefix 된 publish URL → /ebd/<id> 로 정규화', () => {
    expect(
      toNotionEmbedUrl(
  ...
  test('dashed UUID → hex32 로 통일하여 /ebd/<id>', () => {
    expect(
      toNotionEmbedUrl(
