...
// AMOUNT: 할인 금액(원), RATE: 할인율(%) — 서버 CouponDiscountType과 1:1
type CouponDiscountType = 'AMOUNT' | 'RATE';
...
const CouponEditor = ({ editorMode }: CouponEditorProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newValue: CouponRequestValue = {
      ...
      name: value.name,
      code: value.code,
      // 할인율은 전액 개념 없음(퍼센트 그대로), 금액 할인은 전액이면 -1
      discount:
  ...
  const isSubmitDisabled =
    ...
    !value.name ||
    !value.code ||
    (value.discountType === 'AMOUNT' && !isAllDiscount && !value.discount) ||
    isRateOutOfRange ||
          ...
          <div className="flex flex-col flex-wrap gap-4">
            ...
            <div className="flex flex-col flex-wrap gap-3">
              ...
              <div className="flex items-center">
                <label className="flex h-[3.5rem] w-[8.3rem] items-center">
                  할인 방식
                </label>
                <RadioGroup
                  row
                  ...
                  onChange={(e) => {
                    // 할인 방식 변경 시 전액 해제 + 입력값 초기화
                    setIsAllDiscount(false);
                    setValue({
                      ...value,
              ...
              {value.discountType === 'AMOUNT' ? (
                ...
                <div className="flex flex-col gap-1">
                  <div className="flex items-center justify-between">
                    {/* 위 '쿠폰 금액' 입력과 좌우 정렬 맞추기 위한 빈 자리 */}
                    <div className="h-[3.5rem] w-[133px]" />
                    <Input
                      type="number"
                      label="할인율 (%)"
                      placeholder="1~100 사이 값을 입력해주세요."
                      fullWidth={false}
                      className="flex-1"
                      name="discount"
                  ...
                  {value.discount &&
                    (Number(value.discount) < 1 ||
                      Number(value.discount) > 100) && (
                      <p className="text-xxsmall12 text-system-error pl-[133px]">
                        할인율은 1~100 사이 숫자로 입력해주세요.
                      </p>
                    )}
                </div>
              )}
