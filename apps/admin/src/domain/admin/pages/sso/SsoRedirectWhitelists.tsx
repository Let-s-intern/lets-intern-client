...
const SsoRedirectWhitelists = () => {
  const { whitelists, isLoading, isError, isToggling, toggleActive } =
  ...
  return (
    ...
    >
      <p className="text-neutral-40 px-3 text-sm">
        SSO 로그인 후 이동을 허용할 주소 목록입니다. 비활성화하면 해당 서비스의
        SSO 로그인이 즉시 막힙니다.
      </p>
          ...
          <tbody>
            {whitelists.map((item) => (
              <tr key={item.id} className="border-b">
                ...
                <td className="px-3 py-2">{formatCreatedAt(item.createdAt)}</td>
              </tr>
            ))}
          </tbody>
        </table>
