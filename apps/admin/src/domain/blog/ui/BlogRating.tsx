...
const BlogRating = ({ blogId }: BlogRatingProps) => {
  ...
  return (
    <div className="flex w-full flex-col gap-y-4">
      <div className="border-neutral-80 flex w-full flex-col items-center justify-center gap-y-3 rounded-md border-2 px-10 py-8">
      ...
      {starRating && (
        <div className="bg-primary-10 flex w-full flex-col gap-y-3 rounded-md px-10 py-8">
          <div className="flex w-full flex-col items-center justify-center gap-y-4">
            <h3 className="text-xsmall16 text-primary font-bold">
              더 필요한 콘텐츠가 있다면 알려주세요!
            </h3>
            <input
              className={`text-xsmall14 w-full rounded-md border-none ${formValue.length === 0 ? 'bg-neutral-95' : 'bg-[#5177FF]/10'} p-3 outline-none placeholder:text-black/35 ${isPostedRating ? 'text-neutral-45 cursor-not-allowed' : ''}`}
          ...
          <button
            className={`blog_form border-primary text-primary-dark flex w-full items-center justify-center rounded-sm border-2 px-4 py-1.5 ${formValue.length === 0 || isPostedRating ? 'cursor-not-allowed opacity-40' : ''}`}
            onClick={
