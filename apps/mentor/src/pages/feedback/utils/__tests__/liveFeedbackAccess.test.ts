...
import { resolveLiveFeedbackAccess } from '../liveFeedbackAccess';

describe('resolveLiveFeedbackAccess', () => {
  ...
  describe('meetingUrl 이 있는 경우', () => {
    const url = 'https://meet.jit.si/lc-feedback-room8';
    ...
    it('T-10 이전 → pending + url 가림', () => {
      ...
      expect(
        resolveLiveFeedbackAccess(
          ...
          new Date('2026-05-20T10:30:00+09:00'),
        ),
    ...
    it('T-10 이내(시작 5분 전) → active + url 노출', () => {
      ...
      expect(
        resolveLiveFeedbackAccess(
          ...
          new Date('2026-05-20T10:55:00+09:00'),
        ),
    ...
    it('정확히 T-10 시각 → active', () => {
      ...
      expect(
        resolveLiveFeedbackAccess(
          ...
          new Date('2026-05-20T10:50:00+09:00'),
        ),
      ).toEqual({ state: 'active', url });
    });
    ...
    it('endAt 시각 도달(11:30) → ended', () => {
      expect(
        resolveLiveFeedbackAccess(
          ...
          new Date('2026-05-20T11:30:00+09:00'),
        ),
  ...
  it('startDate 가 잘못된 ISO → pending 폴백', () => {
    const r = resolveLiveFeedbackAccess(
