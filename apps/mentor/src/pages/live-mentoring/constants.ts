 ...
 * 하단 고정 플로팅 바의 바깥 래퍼.
 *
 * 왼쪽 사이드바(296px)만 빼고 **본문 전체의 가운데**에 둔다.
 *
 * 화면 전체 기준으로 중앙정렬하면 사이드바 때문에 왼쪽으로 치우쳐 보인다. 반대로
 * 오른쪽 미리보기 컬럼까지 빼면 편집 카드 안에 갇혀 화면 왼쪽에 몰린 것처럼 보였다 —
 * 바가 화면에 하나뿐인 물건이라 본문 가운데가 맞다.
 *
 * 미리보기 위로 걸치는 문제는 미리보기 높이 쪽에서 자리를 비워 해결한다
 * (`LiveMentoringSettingsPage` 의 `lg:h-[calc(...)]`).
 *
...
export const FLOATING_BAR_WRAP =
  'fixed bottom-6 left-0 right-0 z-40 flex justify-center px-4 md:px-8 lg:left-[296px]';
