 ...
 *
 * "상세 수정하기" 버튼은 오픈이 **이미 닫혀 있을 때만**(`canStartEdit`) 보인다.
 * 그래서 "수정하면 오픈이 내려간다"고 적으면 안 된다 — 이미 내려가 있는 상태에서
 * 누르는 버튼이라 사실이 아니다. 실제로 일어나는 일은 상품이 초안으로 돌아가는
 * 것이고, 그 결과 다시 공개하려면 오픈 설정에서 처음부터 다시 오픈해야 한다.
 */
export const START_EDIT_CONFIRM = {
  title: '상세 페이지를 수정할까요?',
  description:
    '수정을 시작하면 상품이 초안으로 돌아가 상세 페이지를 다시 편집할 수 있어요. 수정을 마친 뒤에는 오픈 설정에서 다시 오픈해야 공개됩니다. 진행시간·기간·타입만 바꾸는 거라면 이 버튼 대신 오픈 설정에서 바로 고치고 다시 열면 돼요.',
  confirmText: '수정하러 가기',
  cancelText: '그만두기',
} as const;
