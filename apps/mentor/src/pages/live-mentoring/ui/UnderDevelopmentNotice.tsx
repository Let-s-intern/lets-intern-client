 ...
 *
 * ── 왜 있는가 ────────────────────────────────────────────────
 * 상세 페이지 설정은 아직 MSW 목(`VITE_API_MOCKING=enabled`)으로만 동작한다.
 * 목이 꺼진 환경에서는 서버에 엔드포인트 자체가 없어 조회가 실패하는데, 그때 화면이
 * "고장난 것"처럼 보였다 — "템플릿을 불러오는 중..." 에서 영원히 멈춘다.
 * 개발 중임을 명시하고 담당자를 알리기 위한 임시 안내다.
 * (목이 붙어 데이터가 내려오면 이 안내 대신 실제 화면이 그대로 렌더된다.)
 *
 * 정산 현황 화면도 같은 이유로 이 컴포넌트를 썼었지만, 화면 자체가 폐지되어
 * (오픈 현황·정산 현황 제거, 오픈 설정 상단으로 통합) 더 이상 해당하지 않는다.
 *
 * ── 언제 지우는가 (삭제 조건) ─────────────────────────────────
 * `GET /mentor/live-mentoring/template` 이 실서버에서 정상 응답하면 즉시.
 *
 * ── 무엇을 지우는가 (삭제 목록) ───────────────────────────────
 ...
 * 3. `api/live-mentoring/liveMentoring.ts` 의 `useLiveMentoringTemplateQuery`
 *    `retry: false`. 없는 엔드포인트를 3회 재시도하느라 안내가 늦게 뜨는 걸 막으려던
 *    것이라, API 가 생기면 기본 재시도(3회)로 되돌리는 편이 낫다.
...
interface UnderDevelopmentNoticeProps {
  /** 어떤 데이터를 못 불러왔는지 (예: '상세 페이지 설정'). 생략하면 문구에서 뺀다. */
  feature?: string;
}
