...
describe('MentorSidebar', () => {
  it('재구성된 메뉴 라벨이 모두 노출된다', () => {
    // 피드백 그룹을 펼친 상태(/)로 진입해 하위 라벨까지 검증
    renderSidebar('/');
  ...
  it('피드백 캘린더(/) 진입 시 피드백 그룹이 자동 펼쳐지고 캘린더가 활성 표시된다', () => {
    renderSidebar('/');
  ...
  it('피드백 내역(/feedback-management) 진입 시 그룹이 펼쳐지고 내역이 활성 표시된다', () => {
  ...
  it('LIVE 자식 라우트 진입 시 그룹이 펼쳐지고 LIVE 하위 메뉴가 모두 노출된다', () => {
    renderSidebar('/feedback/live-availability');
  ...
  it('피드백 그룹 밖 라우트에서는 그룹이 접혀 자식이 노출되지 않는다', () => {
    renderSidebar('/profile');

    const groupButton = screen.getByRole('button', { name: /피드백/ });
    expect(groupButton).toHaveAttribute('aria-expanded', 'false');
    expect(screen.queryByText('피드백 캘린더')).not.toBeInTheDocument();
    expect(screen.queryByText('피드백 내역')).not.toBeInTheDocument();
  });

  it('피드백 그룹 토글 버튼으로 펼침/접힘이 전환된다', async () => {
