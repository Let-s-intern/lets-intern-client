...
description: >-
  렛츠커리어 백엔드(Spring Boot) 서버 코드를 직접 읽어 *실제 동작*을 확인할 때 쓰는 스킬. 서버 레포는 클라이언트 레포
  기준 형제 폴더 `../lets-career-server` 에 있다. 비즈니스 로직·트랜잭션 흐름, DB 저장 구조(Entity·연관관계),
  커스텀 쿼리(QueryDSL) 구현, 도메인 에러 코드, 또는 Swagger 스펙과 실제 구현이 어긋나는지 교차검증이 필요할 때
  자동 활성화한다. "서버에서 어떻게 처리하는지", "이 로직 왜 이렇게 동작해", "엔티티가 DB에 어떤 컬럼·연관관계로 저장돼",
  "쿼리 구현 보여줘", "Swagger 말고 실제 소스 확인" 같은 요청이면 추측하지 말고 이 스킬로 실제 소스를 읽어라.
  1544개 파일을 grep 으로 헤매지 말고, 먼저 `references/server-structure.md` 구조 트리로 파일 위치를 바로 짚어
  토큰 낭비를 막아라. 단, 엔드포인트 목록·경로·요청/응답 형태(계약)만 알면 되는 단순 조회는 서버 소스를 뒤지지 말고
  더 싸고 빠른 swagger-api-finder 를 먼저 써라.
---
