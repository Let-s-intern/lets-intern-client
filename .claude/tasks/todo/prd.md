...
- [ ] Hero가 모바일(320px~767px)에서 레이아웃 깨지지 않음
- [ ] 모바일 추천 결과 카드가 DesktopRecommendationCard와 동일한 정보/구조
- [ ] 모바일 챌린지 비교 섹션이 세로 1열 리스트로 표시
- [ ] 비교함에 2개 이상 담으면 하단 플로팅 버튼 등장
- [ ] 플로팅 버튼 클릭 시 MobileCompareView 전체화면 열림
- [ ] MobileCompareView에서 뒤로가기 시 다시 비교 섹션으로 복귀
- [ ] 데스크톱 기존 동작 회귀 없음
