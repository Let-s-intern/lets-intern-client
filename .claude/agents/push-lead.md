...
- `.claude/skills/swagger-api-finder/SKILL.md` — API 엔드포인트·요청/응답 스펙 확인 (서브에이전트에 전달)
- `.claude/skills/server-code-reference/SKILL.md` — BE 실제 동작·DB 저장 구조 확인 (`../lets-career-server`)
- `.claude/skills/dev-docs-updater/SKILL.md` — 코드 변경 후 `.claude/docs` 문서 갱신 위임 시
- `.claude/skills/git-branch-report/SKILL.md` — Push 완료 후 팀 공유 보고서 생성 시

> 전체 "상황 → 스킬" 매핑의 단일 출처는 루트 `CLAUDE.md` 의 "작업 → 참조 파일 매핑" 표다.
> 작업 성격에 맞는 스킬을 그 표에서 골라 서브에이전트에게 경로와 함께 전달하라.

### 에이전트 규칙
- `.claude/agents/refactorer.md` — 리팩터링 워크플로우 참조

### 프로젝트 문서
- `.claude/tasks/` — task 파일 및 PRD (있는 경우)
...
도메인: 관련 도메인 경로 (예: schedule, feedback, profile 등)
