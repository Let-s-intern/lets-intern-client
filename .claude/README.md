...
├── hooks/                 # 자동화 훅 스크립트
├── troubleshooting/       # 원인 미상·미해결 이슈 추적 로그 (가설→검증)
...
| **swagger-api-finder** | Swagger API 엔드포인트·요청/응답 스펙 저비용 탐색 | 자동 (관련 작업) |
| **server-code-reference** | 백엔드(Spring) 실제 소스 확인 (BE 동작·DB·쿼리) | 자동 (관련 작업) |
| **git-branch-report** | 브랜치 작업 → 팀 공유 흑백 HTML/PDF 보고서 | 수동 (`/git-branch-report`) |
| **dev-docs-updater** | `.claude/docs` 개발문서 갱신 + CLAUDE.md/README 3계층 동기화 | 자동 (관련 작업) |

다른 프로젝트로 골격을 옮길 때, 스택과 무관한 스킬(`code-quality`, `folder-structure`, `code-review`, `task-*`, `skill-creator`, `git-branch-report`)은 그대로 가져가고, 이 레포에 특화된 스킬(`vercel-react-best-practices`, `seo`, `swagger-api-finder`·`server-code-reference`(렛츠커리어 BE/API 의존), `dev-docs-updater`(이 `.claude/docs` 구조 의존))은 선택적으로 가져간다.
