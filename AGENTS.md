# Repository Guide (.claude/ 길잡이)

> 모든 AI 에이전트(Claude, Codex, Copilot 등) 가 프로젝트를 빠르게 탐색하기 위한 인덱스.
> 상세 규칙·프로젝트 가이드라인은 `CLAUDE.md` 와 `.claude/behavioral.md` 에 있음.
...
.claude/
├── README.md                             # 디렉토리 개요
├── behavioral.md                         # 필수 행동 규칙 (Claude 자동 로드)
│
├── agents/                               # 서브에이전트 정의
...
| 파일 | 내용 |
|---|---|
| `CLAUDE.md` | 프로젝트 규칙 (도메인 구조, 코드 스타일, 성능, 테스트, 커밋 컨벤션 등) |
| `.claude/behavioral.md` | 필수 행동 규칙 (Claude 자동 로드) |
