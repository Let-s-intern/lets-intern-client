...
interface VodBasicInfoSectionProps {
  vod: VodPublicViewModel;
}

const VodBasicInfoSection = ({ vod }: VodBasicInfoSectionProps) => {
  const thumbnail = vod.thumbnail ?? vod.desktopThumbnail ?? null;
  const price = vod.price ?? 0;
  const discount = vod.discount ?? 0;
    ...
    <div className="mx-auto w-full max-w-[1000px] px-5 pb-10 md:px-10 md:pb-20 md:pt-[60px] lg:px-0">
      ...
      <div className="flex flex-col items-stretch gap-6 md:flex-row">
        ...
        <div className="flex w-full items-center justify-center overflow-hidden rounded-md bg-neutral-95 md:w-3/5">
          <img src={thumbnail ?? undefined} alt="VOD 썸네일" />
        </div>
          ...
          <div className="flex w-full items-center justify-center rounded-md bg-neutral-95 px-4 py-5">
            <div className="flex w-full flex-col gap-y-5 text-primary-90">
              <BasicInfoRow
                icon={<FolderIcon />}
                title="자료 구성"
                content={vod.contentComposition}
              />
              <BasicInfoRow
                icon={<FileIcon />}
                title="열람 방식"
                content={vod.accessMethod}
              />
              <BasicInfoRow
                icon={<UserIcon />}
                title="추천 대상"
                content={vod.recommendedFor}
              />
            </div>
          </div>
          <div className="flex w-full items-center justify-center rounded-md bg-neutral-95 px-4 py-5">
...
export default VodBasicInfoSection;
