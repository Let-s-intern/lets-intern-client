...
const AdditionalDropdown = ({ values, setValues }: Props) => {
    ...
    <div className="relative w-32">
      ...
      >
        <span className="text-sm">
          {topicToText[values?.essentialContentsTopic] || '없음'}
        </span>
        <i>
          <IoMdArrowDropdown />
        </i>
        ...
        <ul className="absolute bottom-0 z-50 w-full translate-y-[calc(100%+0.25rem)] overflow-hidden rounded-lg border border-[#A3A3A3] bg-white">
          {Object.keys(topicRequiredToText).map((topic: any, index: number) => (
            <li
              ...
              onClick={() => {
                setValues({ ...values, essentialContentsTopic: topic });
          ...
          <li
            className="cursor-pointer px-3 py-2 text-sm hover:bg-gray-100"
            onClick={() => {
              setValues({ ...values, essentialContentsTopic: null });
              setIsMenuShown(false);
            }}
          >
            없음
