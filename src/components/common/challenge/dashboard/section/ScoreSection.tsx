  ...
  return (
    <section className="relative flex w-[calc((100%-12px)/2)] flex-col gap-y-4 rounded-xs border border-[#E4E4E7] p-4">
      ...
      {currentScore < 80 || !isProgramDone ? (
        <button
          className={`flex cursor-not-allowed items-center justify-center rounded-xs border-2 border-neutral-80 px-4 py-2.5 text-xsmall14 font-medium text-neutral-50 outline-none`}
          onMouseEnter={() => {
        ...
        <button
          ...
          className={`flex items-center justify-center rounded-xs border-2 border-primary px-4 py-2.5 text-xsmall14 font-medium text-neutral-0 outline-none`}
        >
          수료증 발급
        </button>
      )}
